...
def pipeline_pred(
    ...
    if limit_of_df is None:
        ...
        input_df = spark.read.option(
            "header",
            "True",
        ).csv(path, schema=meta_data["schema"]["train_schema"]).limit(limit_of_df)
    else:
        raise ValueError("Invalid Limit value Provided!")
    ...
    output_cols = ["customer_ID", "prediction"]
